...
{
	output = realloc(output,strlen(s)+2);
	memset(output, 0xaa, strlen(s)+2);
    expected = s;
}
...
static void given(int charsWritten)
{
    ...
    TEST_ASSERT_BYTES_EQUAL('\0', output[strlen(expected)]);
}
...
{
    expect("The number is 6\n");
    given(sprintf(output, "The number is %d\n", 6));
}

TEST(sprintf, InsertIntegerLeadingZeros)
{
    expect("The number is 006\n");
    given(sprintf(output, "The number is %03d\n", 6));
}
#endif  
